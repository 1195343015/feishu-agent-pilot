...
// PPT生成测试接口，直接生成本地PPT文件下载
app.get("/test/ppt/generate", async (_, reply) => {
  try {
    const testSlides = [
    ...
    const pptBuffer = await generatePptFile("PPT生成测试演示", testSlides);
    
    reply.header("Content-Type", "application/vnd.openxmlformats-officedocument.presentationml.presentation");
    reply.header("Content-Disposition", "attachment; filename=test_ppt.pptx");
  ...
  for (const slide of slides) {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      if (currentPageContent.length === maxParagraphsPerPage || i === paragraphs.length - 1) {
        const textContent = currentPageContent.map(p => `• ${p}`).join("\n\n");
        ...
        targetSlide.addText(textContent, {
            x: 1,
            y: i >= maxParagraphsPerPage ? 1.8 : 1.8,
            w: "85%",
            h: 5,
            fontSize: 16,
            color: "444444",
